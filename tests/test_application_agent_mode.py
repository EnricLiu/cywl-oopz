...
async def test_composition_root_routes_chat_and_provider_command_by_agent_flag(
    ...
    assert {"debug", "init", "recall", "role", "whoami"}.issubset(
...
async def test_composition_root_registers_music_tools_only_when_music_is_enabled(
    ...
    assert "music" not in {command.name for command in disabled.commands.commands}
    ...
    assert "music" in {command.name for command in enabled.commands.commands}
