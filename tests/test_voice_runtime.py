...
async def test_realtime_runtime_streams_audio_and_persists_final_turns() -> None:
    ...
    await provider_session.emit(
        VoiceAssistantAudio(PcmChunk(b"\x00" * 960, PROVIDER_OUTPUT_FORMAT, 20, generation=0))
    )
    await provider_session.emit(VoiceTranscriptFinal("assistant", "你好呀", "assistant-1"))
    await provider_session.emit(
