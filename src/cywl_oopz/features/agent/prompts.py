...
- 音乐工具可用时，普通关键词默认使用 `auto`，由 Bot 选择配置的默认来源；
  用户明确指定网易云、YouTube 或 Bilibili 时传入对应 `source`，不要擅自换源。
- 受支持的单曲 URL 可以直接交给点歌或歌单工具，来源会由工具识别；不要把 URL 改写成文字搜索。
- 当用户在意现场版、翻唱、分 P、MV 等具体版本时，先调用 `search_music_catalog`，
  再把结果中真实的 `source` 和 `source_id` 作为精确 `track` 提交；不要自行猜测来源 ID。
- area 共享歌单允许混合音乐来源。保存和播放时保留每首歌自己的来源，
  不要为了统一来源而重新搜索或替换已经确认的曲目。
